...
ManageSerialPort::ManageSerialPort()
{
	...
	saveStateReceiveData = false;
        ...
        form=new WinComForm(getPort(),extSerialPort.getPortSettings());
        connect(form,SIGNAL(SetPortChange(QString,PortSettings)),
                this,SLOT(SlotPortChange(QString,PortSettings)));
...
void ManageSerialPort::openSetFrame(){
    form->show();

}
void ManageSerialPort::SlotPortChange(QString name, PortSettings s){
setPort(name);
setBaudRate(s.BaudRate);
setDataBits(s.DataBits);
setParity(s.Parity);
setStopBits(s.StopBits);
 emit signalSettingsText(SettingToQString());
}
...
bool ManageSerialPort::open()
{
	...
	if (closeCalled)
	{
        ...
        if(res)
            emit textInfo(tr("Port otwarto"));
        else
            emit textInfo(tr("Blad"));
        enableSending();
        enableReceiving();
        setTimeout(0,20);
        receiveData();
...
/*
ulong ManageSerialP
#include "qserialportsetings.h"ort::getLastError()
{
...
QString ManageSerialPort::SettingToQString(){
    QString temp;
    temp.append("Port: "); temp.append(getPort());
    temp.append("\n Bound Rate: "+getBaudRate()+"\n");
    temp.append("\n Data bits: "+getDataBits()+"\n");
    temp.append("\n Stop bits: "+getStopBit()+"\n");
    temp.append("\n Parity: "+getParity()+"\n");
return temp;
}
